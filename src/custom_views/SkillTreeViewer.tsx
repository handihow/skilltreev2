        ...
        getDoc(compositionRef)
            .then(snap => {
                if (snap.exists()) {
                    ...
                    const skillTreeColRef = collection(db, 'compositions', snap.id, 'skilltrees');
                    ...
                    getDocs(skillTreeQuery)
                        .then((values) => {
                            ...
                            const skillQuery = query(skillsColRef, where("composition", "==", snap.id), orderBy("order"));
                            getDocs(skillQuery)
                                .then(values => {
                                    const skills: ISkill[] = [];
                                ...
                                .catch(err => {
                                    snackbarController.open({
                                        type: "error",
                                        message: "Could not retrieve your skills: " + err.message
                                    })
                                })
                        })
                        .catch((err) => {
                    ...
                    snackbarController.open({
                        type: "error",
                        message: "Could not retrieve your composition"
                    })
                }
            })
            .catch(err => {
                snackbarController.open({
                    type: "error",
                    message: "Could not retrieve your composition: " + err.message
                })
            });
    ...
    return (
        <Box style={{ backgroundImage: `url(${url})`, backgroundPosition: 'center', backgroundSize: 'cover', backgroundRepeat: 'no-repeat', minHeight: '100%' }}>
            <Container maxWidth={"xl"}
                ...
                <IconButton aria-label="delete" size="large" color="info" onClick={() => backToOverview()}>
                    <CloseIcon />
                </IconButton>
                {
                    isLoading || !composition ? <CircularProgress /> :
